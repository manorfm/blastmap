...
def build_server(db_path: Path | None = None, backend: LLMBackend | None = None) -> MCPServer:
    ...
    def describe_runtime_configuration(
        ...
        scopes. Source imports can also be filtered by static declaration-finding state
        and literal availability. Pass repository when service names duplicate."""
        with closing(_conn()) as conn:
            return queries.describe_runtime_configuration(
                ...
                source_import_declaration_statuses, source_import_availabilities,
            )
