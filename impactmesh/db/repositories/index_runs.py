...
service, used by `impactmesh status` to show indexing history and token/cost usage."""
...
def usage_totals(conn: sqlite3.Connection, service_id: int | None = None) -> dict:
    ...
    one service) — the number `impactmesh status` prints alongside the recent-runs list,
