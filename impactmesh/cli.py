...
def _cmd_update(args: argparse.Namespace) -> int:
    ...
    if row is None:
        print(f"error: unknown service {args.service!r} (run `impactmesh list`)", file=sys.stderr)
        return 1
    root = Path(row["root_path"])
    if not root.is_dir():
        print(
            f"error: root path for {args.service!r} no longer exists: {root}\n"
            f"       re-run `impactmesh index <newpath> --service {args.service}` instead.",
            file=sys.stderr,
        )
        return 1
...
def _cmd_serve(args: argparse.Namespace) -> int:
    from impactmesh.mcp.server import build_server
...
_TOP_LEVEL_EPILOG = """\
The impactmesh workflow is index -> ask -> verify:
...
Examples:
  impactmesh index ~/code/my-monorepo --repository-name my-monorepo
  impactmesh serve --backend claude
  impactmesh verify 3 --repository my-monorepo --since a1b2c3d

Run `impactmesh <command> --help` for a runnable example of any single command.
"""


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="impactmesh", epilog=_TOP_LEVEL_EPILOG, formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--version", action="version", version=f"impactmesh {impactmesh.__version__}")
    ...
    p_index = sub.add_parser(
        ...
        epilog=(
            "examples:\n"
            "  impactmesh index ~/code/orders-service\n"
            "  impactmesh index ~/code/my-monorepo --repository-name my-monorepo\n"
            "  impactmesh index . --service custom-name --force\n"
            "  # --stack bypasses auto-detection for a folder no heuristic recognizes\n"
            "  # (e.g. a library/CLI package, not a web microservice):\n"
            "  impactmesh index . --service impactmesh-core --stack python\n"
        ),
    ...
    p_update = sub.add_parser(
        ...
        epilog="example:\n  impactmesh update orders-service\n",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    p_export = sub.add_parser(
        ...
        epilog=(
            ...
            "  impactmesh export mermaid --out docs/\n"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    p_analyze = sub.add_parser(
        ...
        epilog=(
            "examples:\n"
            "  impactmesh analyze \"Add support for Pix in checkout\"\n"
            "  impactmesh analyze \"Add support for Pix in checkout\" --backend claude --db verify/sample_project.db\n"
            "  impactmesh analyze \"xyz internal cleanup\" --hint-services notification-service\n"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    ...
    p_verify = sub.add_parser(
        ...
        epilog=(
            "example:\n"
            "  impactmesh verify 3 --repository my-monorepo --since a1b2c3d --record-feedback\n"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p_verify.add_argument("run_id", type=int, help="run_id from a prior find_change_surface/analyze response")
    p_verify.add_argument("--repository", required=True, help="Repository name, as shown by `impactmesh list`/`--repository-name` at index time")
