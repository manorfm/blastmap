...
from impactmesh.generation import embeddings
from impactmesh.generation.backend_base import LLMBackend
from impactmesh.generation.embeddings import EmbeddingBackend, cosine_similarity
from impactmesh.generation.freshness import compute_freshness
from impactmesh.generation.llm_harness import generate_with_retry, load_prompt, load_schema
from impactmesh.generation.next_queries import NextQueryRecommender
from impactmesh.generation.retrieval import CandidateRetrieval, FallbackRetrieval, KeywordGraphRetrieval, SemanticRetrieval
...
def analyze_change_surface(
    ...
    failures_dir = Path.home() / ".impactmesh" / "failures"

    generation = generate_with_retry(backend, prompt, schema, Path.home() / ".impactmesh", failures_dir, "change-surface")
    if generation is None:
        return ChangeSurfaceBuilder().with_note("change surface synthesis failed; see ~/.impactmesh/failures").build()
