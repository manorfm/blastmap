...
remote endpoint. A retry-policy review similarly reads the local service because its
...
An HTTP-resilience-policy review also inspects the local service because client
factories, framework defaults, and runtime configuration can supply the policy.
...
absent.
For a primary service, an indexed internal HTTP call without literal timeout or retry
policy creates an `http-resilience-policy` review. It asks for a documented timeout
policy and repeat-safety before retries, while preserving defaults and factories as
unknown.
...
runtime.
For a primary service with an indexed S3/SQS resource lacking a local encryption
...
mappings.
For a primary service, a source-proven timeout mapping to HTTP 500 creates a
...
enforce remain unknown. Cache and rate-limit context is also returned only for direct
decorators imported from `@nestjs/cache-manager` (`CacheKey`, `CacheTTL`) or
...
`ServiceGrpcKt.*Stub` or `ServiceGrpc.*Stub` property (including a
