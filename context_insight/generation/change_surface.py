...
from context_insight.generation.next_queries import NextQueryRecommender
from context_insight.generation.retrieval import CandidateRetrieval, KeywordGraphRetrieval
...
def analyze_change_surface(
    ...
    failures_dir = Path.home() / ".context-insight" / "failures"

    result = generate_with_retry(backend, prompt, schema, Path.home() / ".context-insight", failures_dir, "change-surface")
    if result is None:
        return ChangeSurfaceBuilder().with_note("change surface synthesis failed; see ~/.context-insight/failures").build()
