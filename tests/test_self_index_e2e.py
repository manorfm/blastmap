"""Self-indexing end-to-end suite: context_insight indexes its own source tree, then
...
discovery path: context_insight's own repo root has no main.py/app.py/wsgi.py (the
Python discovery heuristic's entry-file signal for a web service — see
discovery/python_stack.py's PythonDetector.matches), because context_insight is a
...
called directly by `context_insight update` and by test_orchestrator.py — supplying the
...
SELF_ROOT = Path(__file__).resolve().parents[1] / "context_insight"


def test_indexing_context_insights_own_source_tree_succeeds(tmp_path: Path):
    conn = open_db(tmp_path / "self.db")
    backend = FakeOrchestratorBackend()

    result = index_service(conn, "context_insight-core", SELF_ROOT, PythonDetector(), backend)

    assert result.status == "ok"
    row = services_repo.get_service_by_name(conn, "context_insight-core")
    assert row is not None
    # Dogfooding finding: context_insight has no FastAPI/Flask/Django endpoints, no
...
def test_reindexing_after_a_real_local_edit_only_regenerates_the_changed_unit(tmp_path: Path):
    ...
    backend = FakeOrchestratorBackend()
    index_service(conn, "context_insight-core", SELF_ROOT, PythonDetector(), backend)
    calls_after_first_run = backend.calls

    second_backend = FakeOrchestratorBackend()
    result = index_service(conn, "context_insight-core", SELF_ROOT, PythonDetector(), second_backend)
...
def test_describe_and_search_work_against_the_self_indexed_service(tmp_path: Path):
    ...
    index_service(conn, "context_insight-core", SELF_ROOT, PythonDetector(), FakeOrchestratorBackend())

    described = queries.describe_service(conn, "context_insight-core")
    assert described["name"] == "context_insight-core"
...
def test_find_change_surface_against_self_indexed_knowledge(tmp_path: Path):
    ...
    result = change_surface.analyze_change_surface(
        conn, "Split the repository layer into smaller modules", backend, hint_services=["context_insight-core"],
    )
